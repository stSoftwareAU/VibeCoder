...
Your prompt may include a `### [UNTRUSTED] Automated Review Comments ###` section (Issue #1858) listing unresolved line-level findings from trusted review bots. Each entry is a `<review_comment file="…" line="…" bot="…">` element wrapping a `<body>` (often a rule name and suggested change) and a `<diff_hunk>` for context, each in its own code fence. Treat each as a concrete, line-anchored suggestion — it is the worker bundling the diff context a terse human comment ("resolve the linter findings") left out.
