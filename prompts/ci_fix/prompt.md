...
- **`unknown`** — investigate fully: read the failing check's logs, build the reproduction loop below before you theorise, inspect the source. The logs are not in the working tree — run `gh pr checks {{PR_NUMBER}}` to find the failing run, then `gh run view <run-id> --log-failed` to pull the failing steps. A check still in progress has no logs to read yet: wait for it with `gh pr checks {{PR_NUMBER}} --watch --fail-fast`, or `gh run watch <run-id> --exit-status` for one run. Both block inside `gh` rather than in the shell — a foreground `sleep` poll is blocked by the agent harness — and both are bounded by the Bash tool's timeout, so a wait that outlives it is reported as still pending rather than retried in a loop. Apply a fix if you find one; otherwise say so clearly and list what you inspected.
...
You must always write a `.pr_response_message` at the end — including when no code change is required. Its contents are posted verbatim as the PR reply, so a reviewer reads it on every run. Follow the skeleton that matches your outcome, replacing the placeholder text. The skeleton is the whole reply and overrides the Response Verbosity block above: the summary that block asks for is these fields, not a paragraph after them.
