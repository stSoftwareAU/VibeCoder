...
5. **Drop suppressed and known-open candidates.** Drop any whose stable id
   ...
   cross-link.
6. **Drop in-source suppressions.** If a cited file carries a suppression
   comment recognised by the shared suppression-comment grammar (e.g.
   `<!-- best-practice-ignore: BP-… -->` in Markdown,
   `// best-practice-ignore: BP-…` in code) whose id matches, drop the
   candidate.
