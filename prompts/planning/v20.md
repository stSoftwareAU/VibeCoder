...
## Planning Mode — Draft Stage (Issue #2465, #2652, #3795)
...
<example>
<situation>An issue asks for structured JSON logging and for a log-rotation policy. The `gh issue list` output above shows open issue #812, "Add size-based log rotation", covering the rotation ask.</situation>
<action>Draft one sub-issue for structured JSON logging. For rotation, reference #812 in the draft's summary and in the logging sub-issue's `## Context` instead of drafting a second sub-issue.</action>
