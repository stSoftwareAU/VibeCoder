...
## Planning Mode (Issue #275, #279, #286, #300, #447, #863, #1300, #1408, #2465)
...
5. You **MUST** close this planning issue as the very last step (see "Final step: close this planning issue" below) — Issue #2465
...
7. Do **NOT** add any of these reserved workflow labels to sub-issues:
   ...
   `refine-issue`, `planning`, `question`, `best-model` (Issue #2022).
   The planner account is not on the trusted-author allowlist, so any reserved label it adds is silently stripped by the `label_security` check (Issue #1344) — applying them is wasted effort and can confuse the workflow (Issue #2040).
...
`stSoftwareAU/private-repo-8#59` the gap was nine minutes, long enough for a human
