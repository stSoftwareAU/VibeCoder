...
Before finishing the draft, confirm each proposed sub-issue has testable acceptance criteria, no dependency cycle, no duplicate of an existing open issue, a `Part of #{{ISSUE_NUMBER}}` reference, and a single-PR scope.
