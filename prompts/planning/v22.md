...
Then give the ask list — each ask with the proposed sub-issue covering it, or `Out of scope` and the reason — followed by the suggested implementation order, the dependency relationships, and any assumptions you made. This draft is internal working material — it is **not** posted to the issue. The next turn will adversarially critique and revise it before anything is published.
