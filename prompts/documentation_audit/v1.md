...
2. **Detect** — evidence-backed candidate findings against the eight-check
...
- **Agent instruction files** — `AGENTS.md`, `CLAUDE.md`,
  `.github/copilot-instructions.md`, `.cursorrules`, `GEMINI.md`, and any other
...
## Phase 2 — Apply the eight-check catalogue
...
- **`severity:medium`** — stale, duplicated, or redundant content that is
  wrong-but-not-yet-harmful; an agent file that should be trimmed or deleted; a
...
2. **File the issue** with `gh issue create` (no `--repo` argument) and exactly
   ...
   `🟡 Fold NEAT-AI optimisation learnings into docs/ARCHITECTURE.md, then prune summaries`.
   ...
   - a `## Suggested fix` section describing the concrete change. For a
     ...
     the summary is deleted only _after_ the learning lands. For an agent-file
     finding, say trim-to-link or delete. For a term, give the plain-English
     definition and a Wikipedia link where apt. Where a diagram would help,
     suggest a Mermaid diagram type.
