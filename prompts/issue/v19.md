...
1. Post a comment on issue #{{ISSUE_NUMBER}} explaining what you attempted, why you could not complete it, and exactly what a human needs to do next. Mention `needs-human` in the comment so a trusted human can apply the label to take the issue out of the worker's queue.
2. Do **NOT** apply the `needs-human` label yourself. The worker user is not on the operational-label allowlist, so any label the worker applies is stripped on the next scan and emits a `[SECURITY] [UNTRUSTED_LABEL_CHANGE]` audit event. Request the label via the comment instead.
3. Stop work — do not retry the same failing step.

**Never self-apply any of these reserved workflow labels** — they are managed by humans, not by you. Adding them produces noise without effect:
...
Signal `needs-human` by posting a comment that asks a trusted human to apply the label. Do not apply any of the labels listed above yourself.
