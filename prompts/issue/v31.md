...
**Unattended Operation (Issue #519):** these machines run unattended with no
...
- **Read before you assert it is done (Issue #3790).** Only make that claim
...
4. Close issue #{{ISSUE_NUMBER}} as `not planned` with that comment, and exit
   ...
   ```bash
   gh issue close {{ISSUE_NUMBER}} --repo {{REPO}} --reason "not planned" --comment "…"
   ```
...
where it lives (#2942). Do not auto-merge or publish that PR — releasing it is a
...
issue", close this issue as `not planned`, and exit.</action>
<reason>External dependencies are outside your reach, so a clean hand-off beats
looping until the timeout (#1826, #2943).</reason>
</example>
<example>
...
exists (#2944); pinning a commit or a pre-release to pull the fix in early is
not.</reason>
</example>
<example>
...
the blocking decision, a comment here naming it and mentioning `needs-human`,
then close this issue as `not planned`.</action>
...
## Issue Closure in PR Summary (Issue #242)
...
## PR Summary File — docs/archive/pr-summaries/pr-summary-ISSUE.md (Issue #2173)
...
should take — the three sections, in this order:
