...
**CRITICAL — Unattended Operation (Issue #519):** These machines run unattended
with no human operator monitoring output. You MUST take concrete actions rather
than logging messages or suggestions. Specifically:

- Do NOT just log that an issue "should be closed" or "is already complete" —
  the worker will detect this and auto-close the issue.
- Do NOT suggest that someone should do something — either do it yourself or
  state clearly what you found so the worker can act on it.
- Every run costs time and money. If you determine the issue is already
  resolved, say so explicitly (e.g., "The implementation is already complete" or
  "This has already been fixed") so the worker can close the issue and move on.
...
5. **No gain = close the issue** — if there is no meaningful improvement, **do
   ...
   - Close the issue as `not planned`. State clearly: "This is a negative result
     — no measurable improvement was found."
...
6. Make sure all your changes are committed with clear commit messages
   referencing issue #{{ISSUE_NUMBER}}. {{CODING_GUIDELINES}}
...
_external_. Reuse the existing #1613 classification: a dependency whose source
...
2. Open a single follow-up issue in the relevant repository capturing the
   ...
   silently stripped by the `label_security` check (Issue #1344) — applying it
   is wasted effort (Issue #2040). Keep "mention `needs-human`" as wording in
   the comment, not a self-applied label.
...
4. Close issue #{{ISSUE_NUMBER}} as `not planned` with that comment, and exit
   cleanly — do not retry the original change.
...
3. **Screenshot failures**: If Playwright MCP is unavailable or the page fails
   ...
   visual result and reference the test output as evidence.
...
- **UI Changes**: Capture a screenshot via Playwright MCP (`browser_navigate`
  then `browser_take_screenshot`). Save to `docs/evidence/` (the convention) and
  reference in your PR summary as `![Description](docs/evidence/filename.png)`.
  Describing visual changes in words alone is not sufficient — capture an actual
  screenshot.
...
A soft validation gate (#2229 + #2230) runs at PR-creation time: it warns on
...
Every PR MUST explicitly reference the issue it closes. This ensures the GitHub
...
canonical home for every PR summary — keep it out of `docs/` root (Issue #2173).
...
a glance. Use a fenced `` ```mermaid `` block, for example:
