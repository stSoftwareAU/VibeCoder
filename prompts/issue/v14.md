...
1. Add the `needs-human` label to issue #{{ISSUE_NUMBER}} using `gh issue edit {{ISSUE_NUMBER}} --repo {{REPO}} --add-label "needs-human"`. Create the label first with `gh label create` if it does not exist.
...
Use `needs-human` — and only `needs-human` — to hand work back to a human. Do not use `help wanted` to "stop looping"; use `needs-human`.
