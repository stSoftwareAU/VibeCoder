...
## Tool Usage
Proactively use available tools rather than reasoning about what they would return. Specifically:
- **`gh` CLI**: Use for all GitHub operations — viewing issues, listing PRs, creating PRs, checking CI status. Do not guess at repository state; query it directly.
- **Playwright MCP**: Use `browser_navigate` and `browser_take_screenshot` to capture evidence for any UI or visual changes. Save screenshots to `docs/evidence/`.
- **Read/Edit/Grep tools**: Use to explore and modify the codebase. Read files before editing them.
...
- **Performance Changes**: Include benchmark results showing before/after measurements. If no measurable improvement can be demonstrated, document this in the PR.
