# Orphan / Unmaintained-Dependency Detection (v1)
...
ID prefix (e.g. `ORPHAN-DEPRECATED`). Normalise whitespace and version
suffixes so the same root cause yields the same id across runs.
