# Orphan / Unmaintained-Dependency Detection (v2)
...
- **Dormant-package-then-republished compromise** → `security-scan`
  (idle-task template #1) owns the active-compromise angle of a package
...
- **Posture / readiness** (no SBOM, no CI vuln-scan, no emergency-bump
  runbook) → `supply-chain-readiness` (#5). Cross-link only.
...
5. **Drop in-source suppressions.** If the manifest line has a
   host-language suppression comment of the form
...
`worker/deno/lib/orphan_deps_finding_id.ts` — the shared source of truth
the native pre-filer also uses.

In-source suppression markers use the `best-practice-ignore: BP-…`
grammar (`worker/deno/lib/suppression_comments.ts`) — the same marker
shape the best-practices, test-audit, github-actions-audit, and
supply-chain-readiness templates honour — typically as a comment above
the offending manifest line. The explicitly-named synonym
`orphan-deps-ignore: BP-…` is also honoured for orphan-deps findings;
either marker silences the finding whose `BP-` id matches.
