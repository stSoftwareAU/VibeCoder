...
| `SCR-DEP-REVIEW`       | this template       | repos that accept PRs  | medium   | `dependency-review-action` (or equivalent) runs on every pull request and blocks new high-severity advisories. Evidence: `.github/workflows/*.yml` step calling `actions/dependency-review-action@<sha>`. |
...
- **`SCR-DEP-REVIEW`.** Stay silent on repos that do not accept
  external pull requests (private internal repos with no community
  contributors). Cite a workflow that triggers on
