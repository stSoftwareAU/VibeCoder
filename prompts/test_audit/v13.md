# Test-Audit — Static Test-Suite Maintainability and Coverage-Gap Audit (v13)
...
times the same work at two input sizes and asserts on how the cost
grew (size N vs 4N, as `assertLinearGrowth` in
`worker/deno/tests/support/growth.ts` does) is guarding super-linearity
by shape, not by clock: a slower machine inflates both readings and the
test stays green, which is the very objection this check exists to
raise. Catastrophic backtracking has no wrong output, only a runtime
one, so these are the few tests that must measure. The distinguishing
question is what the elapsed time is compared against — another reading
of the same work is fine; a constant is the defect.
