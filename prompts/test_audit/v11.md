# Test-Audit — Static Test-Suite Maintainability and Coverage-Gap Audit (v10)
...
Walk the test files inventoried in Phase 1 against checks 1–6 and 8–10
...
```
   ...
   - `## Suggested fix`. For a maintainability finding (checks 1–6 and
     8–10), name **both** valid resolutions: (a) rewrite the test to
     ...
     check 9 collapse the duplicates into one data-driven test — or
     (b) delete it, which for a check 10 framework-guarantee test is
     usually the right answer. Sketch the rewritten behaviour-based
     (WHAT) assertion where possible.
