...
request**. Per the #2903 Q2 decision and the "no risky auto-PRs unless
...
2. **Use the repo's native toolchain, in check mode, with no network.**
   ...
   **Never regress a Deno repo to Node tooling (Issue #2222).** If the
...
4. **Drop in-source suppressions.** If a repo-root config or the primary
   workflow carries a host-language suppression comment of the form
...
3. **File the issue.** Call `gh issue create` (no `--repo` argument)
   ...
   - **body** — Markdown. Must contain:
     ...
     - A `## Suggested fix` section recommending the smallest concrete
       ...
       classified as Deno (Issue #2222) — never recommend Node tooling
       for a Deno repo.
