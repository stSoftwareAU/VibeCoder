...
- The GitHub token has been created with appropriate scopes, and is scoped to the repositories you monitor
- The allowed authors create issues in good faith, and their accounts have two-factor authentication enabled
...
flowchart TD
    API["🐙 GitHub API\n(Source of Truth)"]
    L1["🔐 Layer 1: Author Verification\n• Issue author must be in allowed_authors\n• OR work-on label added by allowed author"]
...
| Scope | Purpose |
|-------|---------|
| `repo` | Clone repositories, create branches, push commits |
...
- [ ] Review `allowed_authors` - only these users can trigger work
- [ ] Review `AUTHORIZED_COMMENTERS` - these users can trigger PR fixes
...
- **The rule names what it governs, and covers the whole prompt** ([#3814](https://github.com/stSoftwareAU/VibeCoder/issues/3814)): `buildBoundaryIntegrityInstruction(boundaryId, untrustedBlocks)` takes the names of the blocks the caller actually fenced, so a CI-fix prompt says "the CI console-log excerpt" where an issue prompt says "the issue title, labels, and description" — the fixed issue wording named content that was absent and omitted content that was present. The scope reads "anywhere in this prompt" rather than "above", because a template placeholder renders a fenced block *below* the instruction (the `ci_fix` log excerpt does exactly that). The workflow-setup builder, the one surface that fenced content and emitted no rule at all, now emits one. Values that arrive from the repository but carry no fence — custom instructions, the activity summary, the language and default-branch scalars — are wrapped by a single tagging helper and scrubbed, so none of them can read as prompt-authored instruction text
- **The milestone branch and title are fenced, not spliced** ([#16](https://github.com/stSoftwareAU/VibeCoder/issues/16)): a collaborator with milestone-create/rename (triage) access could name a milestone so that the derived branch — or the title itself — read as an imperative once spliced into the "you MUST target the milestone branch" / "You MUST assign every sub-issue" blocks. Scrubbing alone neutralises forged delimiters, not instruction-shaped prose, and `untrustedBlocks` named neither value, so nothing in the prompt's structure marked them as data. Both are now rendered exactly once inside this run's nonced untrusted fence (`fenceUntrustedIssueText()`), the surrounding instruction refers to them through `<milestone-branch>` / `<milestone>` placeholders, and the boundary-integrity instruction names "the milestone branch" / "the milestone title" whenever the block is present
...
- **Milestone branch and title are fenced, not just scrubbed** ([#16](https://github.com/stSoftwareAU/VibeCoder/issues/16)): a collaborator with milestone create/rename access — a lower trust tier than a committer — controls the milestone title and the branch name derived from it. Both were delimiter-scrubbed and then spliced into imperative instruction blocks ("Use `--base <branch>`", "assigned to milestone **\"…\"**"), so the model read them at the same trust level as the worker-authored text and the prompt's own `untrustedBlocks` list never named them. The scrub neutralises fence forgery, not imperative phrasing. Both values are now wrapped in this run's untrusted fence by `fenceMilestoneValue()`, the surrounding instructions refer to them through the `<milestone-branch>` / `<milestone>` placeholders so the value appears nowhere else, and the boundary-integrity instruction names "the milestone branch name" / "the milestone title"
...
- **Trust classification.** The issue author is classified via the shared `classifyCommentAuthor()` (`allowed_authors` + `authorized_commenters`), exactly as comment authors are.
...
- [ ] **Review `allowed_authors` list**: Ensure only genuinely trusted users are listed. On public repositories, these users' issue content is passed directly to Claude without restriction.
...
- [ ] **Review the `allowed_authors` and `authorized_commenters` lists** for any accounts that should be removed
...
- **Only your config triggers work.** Who can trigger code execution is determined by *your* `.config.json` on *your* machine: `allowed_authors` (who can create issues that trigger work) and `AUTHORIZED_COMMENTERS` (who can trigger PR feedback). Those lists are local, secret, and never committed. Bad actors do not have your config or your GitHub token.
- **Public code is read-only for them.** Someone can fork or read the public repo, but that gives them no access to your worker instance, your token, or your allowlists. They cannot trigger execution on your machine by opening an issue on the *VibeCoder* repo; your worker only processes repos *you* configure (e.g. your org's repos), and only when the issue author or commenter is in *your* allowlist.

So: making the repo public does **not** create a path for remote attackers to run code on your machines. Your deployment is protected by the fact that only you control the machine, the token, and the allowlists. The limitations below (Claude permissions, prompt injection, etc.) apply to *trusted* users who are already in your config; they are not about "bad actors in other countries" reaching through the internet to your worker.
