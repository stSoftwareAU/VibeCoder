...
- **Milestone values are fenced, not spliced** ([#16](https://github.com/stSoftwareAU/VibeCoder/issues/16)): a milestone is created or renamed by any collaborator with **triage** access — a lower trust tier than a committer — yet the milestone title, and the branch name derived from it, were only delimiter-scrubbed before being spliced into imperative instruction blocks ("Use `--base <branch>`…", "You **MUST** assign every sub-issue…") and were never named among the fenced blocks. The scrub neutralises fence forgery, not imperative phrasing, so a branch-shaped title reading as a directive arrived at the same trust level as worker-authored text. Both values now go through the shared `fenceUntrustedIssueText()` boundary and are named in the caller's `untrustedBlocks` ("the milestone branch", "the milestone title"); the instruction text carries a `<milestone-branch>` / `<milestone>` placeholder, so no attacker-influenceable byte sits inside a directive
