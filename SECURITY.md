...
- **Milestone values are fenced, not just scrubbed** ([#16](https://github.com/stSoftwareAU/VibeCoder/issues/16)): a milestone title — and the branch name derived from it — is set by a collaborator with triage access, a lower trust tier than a committer. Both were delimiter-scrubbed and tagged (#3814) but still spliced into imperative prose (`--base <branch>`, `assigned to milestone "<title>"`) and never named among the prompt's declared untrusted blocks, so the model had no structural signal that the text was data. `fencedMilestoneValue()` now reproduces the value inside this run's untrusted boundary in a `codeFenceFor()`-sized code fence, the imperative text carries a `<milestone-branch>` / `<milestone>` placeholder instead of the literal value (the shape #2515 already gave the title's `gh issue create` example), and the issue, planning and critique builders name "the milestone branch" / "the milestone title" in `untrustedBlocks`
