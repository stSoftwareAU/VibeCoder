...
- **Indirection no longer hides a spawn.** The gate matched a **literal** `new Deno.Command("gh"/"git", …)` only, so seven `lib/` modules that reached the binary through a variable — `Deno.Command(cmd[0]!, …)` fed by `runner(["gh", …])`, and `runWithTimeout("gh"/"git", …)` — spawned outside the allowlist, the audit journal and the git timeout while the scan reported clean (Issue #1378). All seven now route through `spawnGh`/`runGitCommand`, and both checks additionally flag the generic wrapper called with the literal binary and an indirect construction in a module that hands that binary to a runner, unless the module imports its chokepoint. The two remaining exemptions are named in `GH_INDIRECT_KNOWN_GAPS`/`GIT_INDIRECT_KNOWN_GAPS` with follow-up #1396; those sets must shrink, never grow.
