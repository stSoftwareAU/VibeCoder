...
- **The guard module is read from the read-only checkout, not the staged copy.** `container/entrypoint.sh` stages `worker/deno` onto VM-local storage and marks the copy writable, because the next launch's `rm -rf` cannot empty a tree with no write bit (Issue #514) — and the uid that write bit belongs to is the one the agent's unrestricted Bash runs as. The wrapper spawns a fresh guard child per call, so the entry point is re-read from disk on every `gh` and `git` the agent runs: resolving it against `import.meta.url` pointed the boundary at a file the constrained party could rewrite mid-launch. Both `defaultGuardModulePath()` and `defaultGitGuardModulePath()` now resolve through [`worker/deno/lib/guard_module_path.ts`](worker/deno/lib/guard_module_path.ts), which names the checkout the launcher mounted read-only (`VIBE_BASE_DIR`, exported beside `PROMPTS_DIR` by the entrypoint) — the driver keeps the fast staged copy, the guard does not. A named checkout that does not carry the module — or a `VIBE_BASE_DIR` that cannot be read at all — is refused like any other uninstallable shim: the install returns `blocked` with `[SECURITY] [GH_GUARD_SHIM_UNAVAILABLE] … [GUARD_MODULE_NOT_IN_CHECKOUT]`, journalled and fail-closed, rather than spawning the agent behind a guard it could rewrite. With `VIBE_BASE_DIR` unset nothing was staged and the running copy *is* the checkout copy, so a host run is unaffected. The staged tree is still writable and still replaced at every launch, so #514 is unaffected. This answers the guard child's `--no-config --no-lock` by location rather than by verification: nothing in its import graph is writable by the agent either.
...
The implementation lives in [`worker/deno/lib/gh_guard_shim.ts`](worker/deno/lib/gh_guard_shim.ts), [`worker/deno/lib/gh_guard_decision.ts`](worker/deno/lib/gh_guard_decision.ts), [`worker/deno/lib/gh_guard_cli.ts`](worker/deno/lib/gh_guard_cli.ts) and — for the issue-lifecycle refusal — [`worker/deno/lib/gh_issue_lifecycle.ts`](worker/deno/lib/gh_issue_lifecycle.ts) with [`worker/deno/lib/claimed_issue_guard.ts`](worker/deno/lib/claimed_issue_guard.ts), and — for the local-state refusal — [`worker/deno/lib/gh_local_state_guard.ts`](worker/deno/lib/gh_local_state_guard.ts), and — for the entry point's own location — [`worker/deno/lib/guard_module_path.ts`](worker/deno/lib/guard_module_path.ts), wired in `claude_runner.ts` (shim) and `issue_worker.ts` (claim), with tests in `worker/deno/tests/gh_guard_{decision,cli,shim}_test.ts`, `worker/deno/tests/claimed_issue_lifecycle_guard_test.ts`, `worker/deno/tests/gh_local_state_guard_test.ts`, `worker/deno/tests/guard_module_path_test.ts` and `worker/deno/tests/gh_body_redaction_test.ts`.
