...
- **Milestone values fenced, not spliced** ([#16](https://github.com/stSoftwareAU/VibeCoder/issues/16)): a milestone title — and the branch name derived from it — is set by any collaborator holding milestone (triage) access, a lower trust tier than a committer, yet it was only delimiter-scrubbed before being spliced into the imperative "Milestone Branch Targeting" / "Milestone Assignment" blocks. The scrub neutralises fence forgery, not imperative phrasing, so a branch-shaped title reading as a directive reached the model at worker-authored trust level. Both values now sit inside this run's untrusted fence (`fenceMilestoneValue()`, code-fenced within the boundary) and the surrounding instructions reference them through the `<milestone-branch>` / `<milestone>` placeholders, so no milestone-derived text is interpolated into prompt-authored prose. `buildBoundaryIntegrityInstruction` names "the milestone branch" / "the milestone title" whenever the block is present
