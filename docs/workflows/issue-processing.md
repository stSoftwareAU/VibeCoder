...
- **Portable handover note (Issue #769):** Preservation saves the run's *code*; the note saves its *intent*. Beside the `wip:` commit the worker writes `.github/handover/issue-<N>.md` into the clone **before** the commit runs, so the same `commitAndPushPending` carries it to the issue branch. It names the interruption cause, the branch, what was done (the commits this run added and the files it left uncommitted), what remains, and whether a wind-down notice was delivered — all from what the worker already knows, so it needs no agent call and works on the timeout path where no agent is alive. Everything in it is provider-neutral: no host paths, no session ids, nothing specific to one agent. Each interruption **rewrites** the note and keeps a short "previous attempts" tail, so a third claim can see two prior runs were interrupted. The path is `.github/handover/` because that is the one hidden directory both safety layers already re-allow: [`gitignore_enforcer.ts`](../../worker/deno/lib/gitignore_enforcer.ts) ignores every other hidden path in a monitored repo and [`pre_commit_safety.ts`](../../worker/deno/lib/pre_commit_safety.ts) refuses to commit one, so a `.vibe/…` note would have been dropped by `git add -A` and lost silently — and, unlike a path under `docs/`, a note left on a WIP branch cannot trip a repo's docs gates and strand the branch it exists to rescue. A failed write is logged and non-fatal: losing the note never costs the code. See [`handover_note.ts`](../../worker/deno/lib/handover_note.ts).
...
```mermaid
flowchart TD
    T["Execute interrupted<br/>(timeout / SIGKILL / SIGTERM)"] --> H["Write the handover note<br/>.github/handover/issue-N.md"]
