...
adopted (Issue #522) and then removed by design, and this section is the record
of that decision so it is not re-proposed.

**The decision.** A published plan is **not** required to name an MVP slice, to
order its sub-issues by value, or to carry a
`No independently valuable slice — <reason>` line. Sub-issues are published in
plain dependency order, and a plan that names no slice closes its parent
normally — no `needs-human` escalation.
...
```mermaid
flowchart LR
    P[Planning run] --> M[Milestone<br/>+ sub-issues]
    M --> B1[Sub-issue PR] --> FB["milestone/&lt;name&gt; branch"]
    M --> B2[Sub-issue PR] --> FB
    FB -->|all milestone issues closed| D[One milestone PR<br/>→ default branch]
    style D fill:#2d6a4f,stroke:#1b4332,color:#fff
```
...
- **Shared work becomes a foundation group** — drafted first, with each
  dependant recording `Depends on: <working title>` as usual.
...
never passes `--milestone` for a group it names there — the worker creates and
assigns those milestones after publish, and `--milestone` on `gh issue create`
stays reserved for the inheritance path where the parent already owns one.
...
`validateMilestoneGroups()` pair, so a prompt can never teach a table the gate
rejects.
