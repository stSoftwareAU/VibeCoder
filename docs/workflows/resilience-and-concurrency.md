...
Every forced reap is emitted as a `container_wedged` self-heal event, so `self-heal-summary` and FLEET-health surface a chronically wedging host instead of losing it in a local log. Diagnosing the VM wedge itself is out of scope — the fleet survives it either way.
...
- **User docs:** [README.md](../../README.md), DEPLOYMENT.md, [issue-processing.md](issue-processing.md), [milestones.md](milestones.md).
