...
without force. Two attempts, at least four hours apart; after that the worker
...
flowchart TD
    ...
    Label --> Budget{"Attempt due and budget left?"}
    Budget -->|No| Sleep
    Budget -->|Yes| Lock{"PR lock acquired?"}
    Lock -->|No — another host holds it| Sleep
    Lock -->|Yes| Record["Comment: attempt N of 2"]
    ...
    Ancestor -->|Yes| Resolved["Comment, drop merge-conflict label"]
    Ancestor -->|No| Abort
    Abort --> Budget2{"Attempts spent?"}
    Budget2 -->|No| Sleep
    Budget2 -->|Yes| Human["Label needs-human + conflict summary"]
...
- **One attempt per PR per 4 hours**, at most **2 attempts**.
- The attempt is recorded as a marker comment on the PR **before** the merge
  starts, so a worker that dies mid-merge still spends its attempt rather than
  looping every pass.
- History lives on the PR, not in host-local state, so the bound holds across
  worker restarts and across fleet hosts.
- A successful merge posts a resolved marker, which resets the budget — a PR
  that conflicts again months later starts from a full budget.
- The final failed attempt applies `needs-human` and posts one summary naming
  the conflicted files and why the merge failed.
