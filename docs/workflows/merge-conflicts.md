...
re-queued (Issue #1773). One restart per originating issue: if the fresh PR conflicts irreconcilably too, that is a human's call
...
- **Four preconditions run before anything is destroyed**, in this order: the
  ...
  it has no *other* open PR of its own; and it can be re-queued *by the
  worker* — it carries the work label already, or the worker is permitted to
  apply one. A failed lookup is never read as an absence.
- **A pickup label the worker may not apply no longer stops the rung**
  ...
  PR is closed, the issue is reopened, it is labelled `needs-human`, and the
  restart comment says **re-apply `<label>` to re-queue this issue**. The
  outcome is `abandoned-unlabelled`, which spends the one restart exactly as a
  plain abandon does, and no `needs-human` goes on the closed PR. Before
  #1773 this declined, and nothing was redone.
