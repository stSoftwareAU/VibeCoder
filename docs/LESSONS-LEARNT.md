...
| Quality gates with real tests | ✅ Implemented | `./quality.sh` runs before every PR (`deno test`, `deno lint`, `deno check`, shellcheck, benchmark audit). TDD is enforced — failing tests must be written first. | [AGENTS.md](../AGENTS.md) (quality gates, TDD), [§ Real unit tests](#-the-importance-of-real-unit-tests) |
