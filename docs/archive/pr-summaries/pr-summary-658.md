...
question** (pre-ticked in our checkbox format), and **facts are yours,
decisions are theirs** (never ask the user something a read of the repository,
`gh`, or the filesystem answers). Committed prompt versions are immutable, so
v13 is untouched and v14 becomes the version the worker loads. Closes #658.
...
- **Frontier vs the ~1500-character mobile bound.** The frontier wins. The
  round compresses instead of dropping questions — one-line stems, at most four
...
- **Pre-ticking changes what a tick means.** Silence now reads as consent, so
  ...
  contest, and the user can overturn any of them in a later round. A question
  is only "unanswered" when it was posted with no recommendation at all.
...
All eight failed before `v14.md` existed except the v13-immutability test,
which passed unchanged throughout — that is the point of it.
...
- Added `worker/deno/tests/grill_me_frontier_test.ts`:
  ...
  - `buildGrillMePrompt - a built round carries the frontier rules and no
    placeholders` — a prompt built through the processor carries the new rules
...
- `docs/workflows/grill-me.md` — the round description no longer says "smallest
  ...
  facts-vs-decisions split. The worked mobile example and both existing
  diagrams were updated to show pre-ticked recommendations and a whole-frontier
  round.
