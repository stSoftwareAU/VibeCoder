...
- **met** — the PR summary states the fail-before / pass-after linkage — evidence: the **Evidence** section above, with the quoted gate refusal — reviewer: partial — reason: the reviewer saw only the diff and the commit, where no PR summary existed yet; the summary was written afterwards and carries the linkage
...
- **unrequested** — assertions beyond the listed set: the unmerged-file list, `git ls-tree -r HEAD`, and `committedNewChanges`/`finalUnpushedCount` — evidence: `worker/deno/tests/commit_and_push_pending_test.ts` in the same test — reviewer: unrequested — reason: cheap preconditions that make a setup failure legible and the tree check unambiguous; none change what is proven
