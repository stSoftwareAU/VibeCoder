...
The run was interrupted after 2079s, so it never reported completion: whatever the issue still asks for beyond the changes above is outstanding.

Diff `issue-1949-ordinary-coding-run-failures-never-reach-the-faile` against its base branch to see the 1 commit(s) and 0 preserved file(s) named above, continue from them, and do not revert them unless they are wrong.
