...
- `docs/audits/security-sweep-1947-callback-run-outcome.md`

## What remains

The run was interrupted after 1756s, so it never reported completion: whatever the issue still asks for beyond the changes above is outstanding.

Diff `issue-1947-post-run-callback-context-carries-only-success-fai` against its base branch to see the 1 commit(s) and 2 preserved file(s) named above, continue from them, and do not revert them unless they are wrong.
