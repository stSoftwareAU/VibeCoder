...
> `rust-v0.147.0`** — the exact release `container/providers/codex.sh`
> downloads and checksums against the `codex` pin in `container/tools.json`.
...
Codex CLI actually exposes, how that was established, and which of it the
worker reads.
...
`should_persist_event_msg` (`codex-rs/rollout/src/policy.rs`) returns `true`
for `EventMsg::TokenCount(_)`, and `TokenCountEvent`
...
which is `#[serde(tag = "type", content = "payload", rename_all = "snake_case")]`,
...
`%b %-d…, %Y %-I:%M %p` on another day: the *host's* local time, with no offset
...
account's own rate limits and billing, so the adapter answers
`api-key-account` — an explicit unknown — rather than inventing a weekly
allowance for the pool to rank on. Only credential *presence* is read; no token
value is ever opened, returned or logged.
...
| Secrets | Reads credential *presence* only; the diagnostic's output is passed through `redactSecrets` |
...
credential *kind* — no token, key, account id, email or credit balance. No
