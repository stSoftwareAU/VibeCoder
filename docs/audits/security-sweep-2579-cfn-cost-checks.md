...
| logical ids | untrusted — template keys | matched by plain string search, never compiled into a regex, so a crafted id cannot shape a pattern; rendered into the scan prompt as backticked text |
