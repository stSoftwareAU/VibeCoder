...
**Issue:** [#2438](https://github.com/stSoftwareAU/VibeCoder/issues/2438) (chunk
top-up-2438) · **Parent:** #1209
...
team slugs the read returned. Three callers reach it — `pr_create_rest.ts`
(which covers `deferred_pr_drain.ts` and the completion phase's REST fallback
transitively), `phases/completion_phase.ts` and `milestone_sync_pr.ts` — each
immediately after a PR is created.
