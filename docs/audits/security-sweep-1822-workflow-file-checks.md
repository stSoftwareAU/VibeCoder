...
**Issue:** [#1822](https://github.com/stSoftwareAU/VibeCoder/issues/1822) (chunk
top-up-1822) · **Parent:** #1209
...
own record documents. The module is claimed by **top-up-1822**, and this file is
the reading of it.
...
already swept — the nine native pre-filers and `workflow_hygiene_check.ts` all
sit in slice 12e. It adds two private normalising functions — `fromScanner` and
`fromHygiene` — that copy four fields off a finding and return a new object. No
scanner logic is reimplemented here.
...
No findings. The accepted residual: the table is a list, so a check omitted from
it is a check the conformance test stops running. That is why
