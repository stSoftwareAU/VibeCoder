...
**Issue:** [#2103](https://github.com/stSoftwareAU/VibeCoder/issues/2103)
(chunk top-up-2103) · **Parent:** #1209

This is the written record for the one module that entered
`worker/deno/lib/` under #2103:
...
claimed by **top-up-2103**, and this file is the reading of it.

## `worker/deno/lib/pr_title_read.ts`

The module makes one read-only GitHub call and returns its trimmed stdout.
It spawns nothing itself, touches no filesystem path, and holds no state.
...
Graft query, and therefore inside whatever bundle Graft selects for it. That
is the same posture #2099 recorded for the query as a whole: the query is one
argv element, and the bundle it produces is rendered by
`formatGraftContextSection`, which redacts secrets, scrubs delimiter-shaped
patterns and wraps the text in a fence it cannot close. A title carrying
prompt-injection text is therefore data behind an untrusted fence, exactly as
the issue body already is on the issue-shaped runs.
