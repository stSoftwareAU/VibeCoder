...
**Issue:** [#2023](https://github.com/stSoftwareAU/VibeCoder/issues/2023) (chunk
top-up-2023) · **Parent:** #1209
...
stage blobs, asks each branch's history whether it ever carried the other side's
exact version of a path (`git log --find-object=<blob>`), and stages the side
that provably contains the other. Every git call goes through `runGitCommand`
(`git_timeout.ts`), the timeout- and audit-guarded chokepoint; the module never
spawns anything itself and writes nothing outside the clone's index and working
tree.
