...
**Issue:** [#1951](https://github.com/stSoftwareAU/VibeCoder/issues/1951) (chunk
12ac) · **Parent:** #1209

The written record for the four modules that entered `worker/deno/lib/` with the
secondary-rate-limit deferral:
...
| the retrying is bounded on both axes           | a non-throttle failure counts up to `MAX_DEFERRED_PR_ATTEMPTS` and is then abandoned. A throttle refusal deliberately does not count — that is the drain doing its job — so the _age_ bounds it instead: past `MAX_DEFERRED_PR_AGE_SECONDS` (24 h) a record is abandoned whatever refused it, and no record can be re-parked for ever |
