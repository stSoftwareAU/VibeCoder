...
**Issue:** [#2495](https://github.com/stSoftwareAU/VibeCoder/issues/2495) (chunk
top-up-2495) · **Parent:** #1209
...
no subprocess, no environment read, no permission beyond what the caller already
holds; every value it touches is passed in.
...
| dependency absent from its repo's open list     | dropped                                 | read as closed, so it cannot block the root. The list is capped at 200 open issues, so a dependency past the cap also reads as closed — which cannot promote blocked work, because a promotion is only acted on for an issue that is already a candidate, and the collector's own `isDependencyBlocked` is what admitted it |
...
Promotion is an **in-memory rank change for the length of one scan**: the module
writes nothing to GitHub, applies no label (the worker cannot apply
