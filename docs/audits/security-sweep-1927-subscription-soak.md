...
**Issue:** [#1927](https://github.com/stSoftwareAU/VibeCoder/issues/1927)
(chunk top-up-1927) · **Parent:** #1209
...
| `auths` | the auth-mode resolver | only the *kind* and flags (`persistedDurably`, `refreshedAt`, `expiresAt`, `reason`) are folded in; the reason is a label, never a token |
...
| secret surface | `kind: "metered"` auth carries only `reason` (e.g. `OPENAI_API_KEY` as a *name*), never the value; `buildSubscriptionSoakStatus` has no parameter that accepts a secret |
