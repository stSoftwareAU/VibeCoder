...
**Issue:** [#2172](https://github.com/stSoftwareAU/VibeCoder/issues/2172) (chunk
top-up-2172) · **Parent:** #1209
...
**top-up-2172**, and this file is the reading of them. They are swept together
because one is the other's only reason to exist: `markdown_table.ts` holds the
table primitives `plan_milestone_groups.ts` and the pre-existing
...
planning run publishes on its parent issue. Modelled on `plan_coverage_gate.ts`
(swept under chunk 12c) and sharing its two security boundaries: the bounded
scan above, and the fleet-author check on every candidate comment.
...
1. **A missing table is not a failure.** The gate landed before the prompts that
   teach the table (#2174), so a parent with no `## Milestones` table takes the
   legacy single-milestone path. The absence is logged at info; it is a
   documented transitional decision, not a swallowed fault.
2. **A structurally broken table escalates _and_ falls back.** The plan is
