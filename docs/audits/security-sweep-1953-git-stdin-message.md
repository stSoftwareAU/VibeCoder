...
| the stream is read exactly once, by one reader | the guard consumes stdin and signals that with its own allow marker, `VIBE_GIT_GUARD_ALLOW_STDIN`; the wrapper then runs the real `git` with `</dev/null`, so no second reader can see an empty or partial message |
