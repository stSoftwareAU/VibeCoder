...
**Issue:** [#2493](https://github.com/stSoftwareAU/VibeCoder/issues/2493) (chunk
top-up-2493) · **Parent:** #1209
...
behind them, the monitored-repo set, the discovery labels, the needs-human label
name and the fleet-author allowlist — walks each chain breadth-first and returns
which chain members to promote, which roots the fleet already holds and which
roots are unworkable. No I/O, no subprocess, no environment read, no permission
beyond what the caller already holds; the only data it touches is what the
caller passes in. Its two imports are `DependencyBlocker` (`import type`, erased
at runtime) and `isFleetAuthor` from `worker/deno/lib/fleet_authors.ts`, which
...
must not report a monitored repo as unreachable and silently drop its subtree.
Every `detail` string is a repo name, a label name, or an assignee login taken
from GitHub data the caller already fenced as untrusted; it is reported, never
interpolated into a command or a path.
