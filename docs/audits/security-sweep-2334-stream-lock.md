...
reads the milestone's open issues with one `gh issue list`, parses the
heartbeat markers and `CLAIM_LOCK` comments the listing carries, and reports
...
| regex safety           | two anchored, quantifier-free patterns (the claim-host capture, and `hostFromMachineId`'s UUID suffix); no nested quantifier, so no backtracking surface                                                                            |
...
| fail direction         | fail open, loudly — a `gh` outage or a malformed payload logs `stream_check_failed` and lets the claim proceed, matching every other pre-claim check. Absence of a listing is never reported as a clean "free"                      |
