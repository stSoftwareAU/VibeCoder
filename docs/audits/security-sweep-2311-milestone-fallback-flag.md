...
goes through the injected `fileMergeFallbackFn` (`merge_fallback_issue.ts`,
swept with Issue #2304) and the one read it makes goes through the sync's
injected `ghCommandFn`.
