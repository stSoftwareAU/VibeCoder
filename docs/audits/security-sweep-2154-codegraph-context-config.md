# Security sweep — the CodeGraph context switch (`codegraph_context_config.ts`)

**Issue:** [#2154](https://github.com/stSoftwareAU/VibeCoder/issues/2154)
(chunk top-up-2154) · **Parent:** #1209

This is the written record for the one module that entered `worker/deno/lib/`
under #2154:

- `worker/deno/lib/codegraph_context_config.ts` — added by #2154.

## Why a new slice rather than a line in an old one

Appending a module to a slice whose sweep ran before it existed is the cheapest
way to make `diffCoverage` green and a false record. The module is claimed by
**top-up-2154**, and this file is the reading of it.

## `worker/deno/lib/codegraph_context_config.ts`

A pure parser: it turns the raw `.config.json` `codegraph_context` block into
...
| ----- | ------ | ----------------- |
| the `codegraph_context` block | operator-supplied JSON, already parsed by `config.ts` | typed `unknown` and narrowed here. A non-object block, and a non-boolean `enabled`, are each refused with an error naming `codegraph_context.enabled`; only `undefined` (the key absent) reads as the default |
