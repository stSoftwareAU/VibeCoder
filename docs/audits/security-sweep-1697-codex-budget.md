...
| a directory it cannot read | ✅ `safeEntryNames` and `readFileTail` answer `[]` / `null` rather than throwing, and a run where *every* candidate file was unreadable is reported `read-error` — never "no budget", and never a silent pass |
...
| exhaustion cannot be inflated into a reset | ✅ `recordExhaustion` sets `remainingFraction: 0` — which explicit exhaustion genuinely evidences — and leaves `resetAt` undefined, because the CLI renders the reset in host-local time with no offset. A guessed instant would let a caller retry into a still-closed window |
| a rejected credential is not zero budget | ✅ `recordAuthRejected` stores `{ known: false, reason: "auth-rejected" }`, so a 401 cannot be ranked as an exhausted subscription |
