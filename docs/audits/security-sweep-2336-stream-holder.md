...
None. No input reaches a shell or an unbounded path; the one filesystem write
is keyed by `streamKey`, which is a single `[a-z0-9_-]` segment carrying no
`/`, no `..` and no leading `.`. Every untrusted value is either filtered by
author, parsed by a pattern that returns `null` on anything unrecognised, or
passed to `gh` as a single argv element.
