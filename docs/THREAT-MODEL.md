...
| **C13** | Agent-subprocess `gh` guard — a PATH shim re-applying the same decision, with the allowlist baked in, unknown roots refused, local `gh` state (credentials, config, aliases, extensions) unwritable, and the guard entry point itself read from the read-only checkout rather than the agent-writable staged worker copy | `worker/deno/lib/gh_guard_shim.ts`, `worker/deno/lib/gh_guard_decision.ts`, `worker/deno/lib/gh_guard_cli.ts`, `worker/deno/lib/gh_local_state_guard.ts`, `worker/deno/lib/guard_module_path.ts` | `worker/deno/tests/gh_guard_shim_test.ts`, `worker/deno/tests/gh_guard_decision_test.ts`, `worker/deno/tests/gh_guard_cli_test.ts`, `worker/deno/tests/gh_local_state_guard_test.ts`, `worker/deno/tests/guard_module_path_test.ts` |
