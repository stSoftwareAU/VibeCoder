...
| **AP-13** | **Secret leakage into a permanent public record** — a token echoed by a subprocess is quoted into a comment, PR body or log that cannot be un-published, or a credential quoted in untrusted text enters the model's own context before any sink sees it | A1, A5 | C23, C24, C32 | `worker/deno/lib/secret_redaction.ts` |
