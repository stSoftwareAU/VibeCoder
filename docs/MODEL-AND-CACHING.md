...
launch rate is now the standard price. Sonnet is not a phase default; it is the
third rung of the `fable → opus → sonnet → haiku` rate-limit ladder, so the row
matters for costing a downgraded run.
