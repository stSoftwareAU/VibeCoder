...
- **A quota pause is not a failure.** A run that stops because this host is out of quota — GitHub's, or a model rate limit whose back-off outlasts the run — exits **75** and writes `quota-pause.json` in the host log directory. Since **Issue #1669** a Claude *subscription* usage limit no longer reaches this path at all: the window belongs to the credential, so the worker records it as spent, switches to another credential in the pool, or — when none has quota — refuses the spawn and carries on with everything that needs no agent. It writes no `usage` rate-limit signal, produces no `quota-pause.json`, and never exits 75 for a Claude usage limit. See [Rate limits and subscription usage limits](TROUBLESHOOTING.md#-rate-limits-and-subscription-usage-limits). That is a scheduled outcome: the failure streak resets, nothing escalates, the container is not treated as suspect, and the supervisor re-probes at a fixed cadence (`VIBE_QUOTA_PAUSE_SLEEP_SECONDS`, default 3600 s) rather than doubling its wait — the quota may be extended before its stated reset. A host that genuinely *crashes* while out of quota writes no marker, so it backs off exactly as above.
