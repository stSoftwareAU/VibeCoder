...
| Model calls to index     | none by default; with `graft_context.deep` the summary pass calls the host's configured provider under its own key (Issue #2315) — Graft reports no token total for it, so it is an indexing cost outside the run's figures | none                                                                                         |
