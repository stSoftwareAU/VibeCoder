...
## Native mode is outside this boundary

Everything on this page describes the default `container` run mode. A host may
opt into the `native` mode instead — `"run_mode": "native"` in `.config.json`,
or `VIBE_RUN_MODE=native` for one run (Issues #4145, #4146, #4148) — and
**native mode is outside the #4060 containment boundary**: there is no
container, so no mount set, no privilege reduction and no network boundary
applies. The worker runs with host access, exactly as the fleet ran before
containment.

That is a deliberate trade an operator makes, not a degraded mode the worker
can slip into: the opt-in is explicit, and container mode never falls back to
it — a missing container runtime stays a loud failure (Issue #3234). The
guarantees on this page are unchanged and undiluted; they simply describe the
mode a contained host runs in. Choose native only when the host genuinely
cannot be contained — see
[`run_mode`](CONFIGURATION.md#-run-mode-issue-4146) for the worked case.

## Seatbelt mode: the boundary without the VM (Issue #4300)

`seatbelt` is a third, macOS-only opt-in — `"run_mode": "seatbelt"` or
`VIBE_RUN_MODE=seatbelt`. It runs the worker natively (full CPU, GPU, unified
memory, no virtiofs, no VM boot) but under a macOS Seatbelt profile
(`sandbox-exec -f`, the mechanism Claude Code's own bash sandbox uses) that is
**deny-by-default** for the filesystem and allows exactly the paths container
mode mounts: the checkout, run-config and credentials read-only; the work dir,
logs, the tools' own caches under `HOME`, and `TMPDIR` read-write; system and
toolchain roots read-only; outbound network. Everything else on the machine —
`~/Documents`, other checkouts, mail, browser profiles, `~/.ssh` — is refused
by the kernel. The profile is generated by `mod.ts seatbelt-profile` from the
same host-path resolver the container plan uses, so the two allowlists cannot
drift apart, and a live test proves a canary in the launcher's own `HOME` is
unreadable from inside.

What it does **not** give you: Seatbelt confines file access, not kernel
attack surface. It is weaker than a VM against a malicious-code escape, so it
is a per-host choice for hosts whose repositories the operator trusts, never
the default; container mode remains for hosts running untrusted code. It also
runs the worker as the operator's own user (like native), so the gh-guard
shim, write-repo allowlist and secret redaction remain the defence-in-depth
they are in every mode.

Why it exists: on the shared 10-core laptop, container mode's 6-vCPU/16 GiB
VM stalled wholesale under host bursts and every substantive run timed out,
while the machine's other four cores and its GPU sat idle. The containment
requirement is the filesystem boundary; seatbelt keeps that and gives the
agent the machine.
...
- **The workspace is a named volume, not a host directory** (Issue #4186).
  ...
  and re-run on every launch. Native mode keeps using the host `$WORK_DIR`
  directly.
...
- **GREEN** — zero host-mode (native/seatbelt) launches, every launch verified
  as container, the observed window at least `--min-window-days` (default 14),
  no regression issue open.
