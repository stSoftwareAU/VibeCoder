...
| **issue**                  | Implement a GitHub issue: read AGENTS.md/README, follow TDD (failing tests first, real “what” tests), handle untrusted issue content safely, run quality checks, commit with clear messages, and produce a PR summary with evidence. Run autonomously (no plan mode); take concrete actions, don’t just suggest. When the issue turns out to be already fixed, declare it with the `vibe-already-resolved` marker and cite the commit/PR and how it was verified, so the worker closes the issue with that evidence instead of escalating it. When the issue body carries a `## Acceptance Criteria` section, the summary must close each criterion out as `met` / `partial` / `missing` with the file or test that evidences it, a one-line reason for every gap, and an `unrequested` entry for any change in the diff not traceable to the issue (from `prompts/issue/v36.md`, Issue #518) — see [Issue processing § Acceptance-criteria closure](workflows/issue-processing.md#-acceptance-criteria-closure-before-the-pr). When the issue carries the `bug` label, the summary must also carry a `## Reproduction` block naming the symptom, the reproduction status as `verified` / `partial` / `not-run`, and the covering regression test — `verified` only when the test was observed failing against the unfixed code and passing after the fix (from `prompts/issue/v37.md`, Issue #521), with the method behind that status — a red-capable command first, minimised into the regression test — stated from `prompts/issue/v38.md` (Issue #661) — see [Issue processing § Reproduction status](workflows/issue-processing.md#-reproduction-status-on-a-bug-fix). A criteria-bearing issue also dispatches two independent reviewer sub-agents before the summary is written — a Spec reviewer given only the diff and the issue body, whose verdict populates the closure block, and a Standards reviewer given the diff and `CODING-STANDARDS.md`, whose findings sit under their own `## Standards Review` heading, never merged or reranked across axes (from `prompts/issue/v39.md`, Issue #663) — see [Issue processing § Independent review on two axes](workflows/issue-processing.md#-independent-review-on-two-axes).                                                                                                                                                                                                                                                                  |
