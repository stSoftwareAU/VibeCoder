...
| **workflow_setup**         | Provision the standard CI/security GitHub Actions workflows in a target repository (lint, test, Dependency Review, Gitleaks secrets scan, Semgrep SAST, private-repo-14 scorer hardening, markdown-lint). SHA-pins every action and writes each generated workflow on a branch behind a PR (Issues #1756, #3799). Built by the `build-workflow-setup-prompt` CLI operation; it has no automated caller today — `setup workflow-sync` audits workflows natively and loads no prompt.                                                                                                        |
