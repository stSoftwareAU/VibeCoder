...
| Prompt                     | Goal                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **coding_guidelines**      | Shared principles and rules embedded into other prompts: KISS (Keep It Simple, Stupid), DRY (Don't Repeat Yourself), Australian English, secure coding principles, and how to use available tools (e.g. `gh`, Playwright). Not used alone; every phase loads its core layer, and only code-writing phases load the code layer (Issue #2574, see [EXTENDING.md § Phase-scoped coding-guidelines layers](EXTENDING.md#phase-scoped-coding-guidelines-layers)).                                                                                                                                                                                                              |
| **coding_guidelines_&lt;provider&gt;[_&lt;model&gt;]** | Optional per-model overlay appended behind the agnostic `coding_guidelines` baseline when the worker knows which agent is running the prompt (Issue #374). Holds working-style guidance that is only true of that provider or model; absent for an identity, the baseline is used unchanged. See [EXTENDING.md § Per-model coding-guidelines overlays](EXTENDING.md#per-model-coding-guidelines-overlays). |
| **issue**                  | Implement a GitHub issue: read AGENTS.md/README, follow TDD (failing tests first, real “what” tests), handle untrusted issue content safely, run quality checks, commit with clear messages, and produce a PR summary with evidence. Run autonomously (no plan mode); take concrete actions, don’t just suggest. When the issue turns out to be already fixed, declare it with the `vibe-already-resolved` marker and cite the commit/PR and how it was verified, so the worker closes the issue with that evidence instead of escalating it. When the issue body carries a `## Acceptance Criteria` section, the summary must close each criterion out as `met` / `partial` / `missing` with the file or test that evidences it, a one-line reason for every gap, and an `unrequested` entry for any change in the diff not traceable to the issue (`prompts/issue/prompt.md`, Issue #518) — see [Issue processing § Acceptance-criteria closure](workflows/issue-processing.md#-acceptance-criteria-closure-before-the-pr). When the issue carries the `bug` label, the summary must also carry a `## Reproduction` block naming the symptom, the reproduction status as `verified` / `partial` / `not-run`, and the covering regression test — `verified` only when the test was observed failing against the unfixed code and passing after the fix (`prompts/issue/prompt.md`, Issue #521), with the method behind that status — a red-capable command first, minimised into the regression test — stated in the same template (Issue #661) — see [Issue processing § Reproduction status](workflows/issue-processing.md#-reproduction-status-on-a-bug-fix). A criteria-bearing issue also dispatches two independent reviewer sub-agents before the summary is written — a Spec reviewer given only the diff and the issue body, whose verdict populates the closure block, and a Standards reviewer given the diff and `CODING-STANDARDS.md`, whose findings sit under their own `## Standards Review` heading, never merged or reranked across axes (`prompts/issue/prompt.md`, Issue #663) — see [Issue processing § Independent review on two axes](workflows/issue-processing.md#-independent-review-on-two-axes). Any file the run adds or changes under `.github/workflows/` is held to the file-scoped Actions checks: the prompt names every `WORKFLOW_FILE_CHECKS` rule, commits a workflow-sync template's YAML verbatim with its already-resolved pins copied as given, and forbids writing an action SHA from memory (`prompts/issue/prompt.md`, Issue #1825). |
...
| **ci_fix** | Fix CI failures on a PR: diagnose the failing check using the failure classifier, apply a minimal fix that addresses the root cause, and do not disable or skip tests. Diagnosis is gated behind a reproduction loop — a red-capable command before any hypothesis, minimised before the fix, three to five ranked falsifiable hypotheses before instrumenting, and `[DEBUG-…]`-tagged instrumentation grepped out before the commit — with a bounded attempt and an honest give-up path (Issue #661); see [CI fix § The reproduction loop](workflows/ci-fix.md#-the-reproduction-loop-before-the-fix). When the same check is failing on the **base branch**, the agent says so and ends its `.pr_response_message` with a `Depends on owner/repo#N` line naming the tracking issue (searching that repo first and filing one only when none exists); the worker verifies the base branch's latest run of that check is red and then defers — one comment, no `needs-human`, no attempt charged (Issue #1880); see [CI fix § Decision points and exceptions](workflows/ci-fix.md#-decision-points-and-exceptions). Used when a PR fails CI. |
...
| **best_practices**         | Run a bucket-scoped best-practices review (one of rust, typescript, react, java, html, aws-cloudformation, terraform, general, design) over a monitored repository, filing findings as `best-practices`-labelled issues. Idle-task template #2 — see [Best-Practices Scans](BEST-PRACTICES-SCAN.md) and `prompts/best_practices/`.                                                                                                                                                                                                                                                         |
...
## One editable template per type

- Each prompt type has exactly one file — `prompts/<type>/prompt.md` — and that
  is what the worker loads at runtime. Browse a directory on GitHub to read it
  (e.g.
  [`prompts/coding_guidelines/`](https://github.com/stSoftwareAU/VibeCoder/tree/main/prompts/coding_guidelines)).
- **Edit it in place.** The old immutable `vN.md` convention was removed in
  Issue #844: it only made sense while the repo was private and history
  travelled in a zip. Git history is now the record —
  `git log -p prompts/<type>/prompt.md`. See
  [CODING-STANDARDS.md § Prompt Templates](../CODING-STANDARDS.md#prompt-templates).
- The worker logs the checkout's short commit hash for each run, which pins the
  exact prompt text that ran (traceability).
