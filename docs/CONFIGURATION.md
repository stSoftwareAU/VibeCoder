...
| `issue_executor_split` | `false` | Whether `issue`-phase runs use the split executor (Issue #2341). Host-wide, and applies to every `issue`-phase run on the host — `failed-once` retries and milestone child issues included. Every other phase (`planning`, `pr_feedback`, `ci_fix`, …) is unaffected whatever this is set to. A `repo_config.<repo>.issue_executor_split` entry overrides it for that repository. Default `false` — an unconfigured host invokes the coding agent exactly as it does today. |
