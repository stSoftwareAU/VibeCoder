...
| Max concurrent issues | `max_concurrent_issues` | `2` | Issue slots worked concurrently per host (integer 1–8). Above `1` the Priority-2 scan runs as a pool, one clone per slot; the memory-pressure governor lowers the effective count (never raises it). `1` opts into the serial loop. |
...
| Issue retry cooldown | `issue_retry_cooldown` | `600` | Seconds to skip a failed issue before retrying (10 minutes). Persisted to disk. Timeout-class failures escalate instead: 2 h → 6 h → 24 h for consecutive timeouts within 48 h, with a `needs-human` handoff on the third. `MIN_CLAIM_RUNWAY_SECONDS` (env, default 300, 0 disables) additionally stops the worker claiming a Priority-2 issue when the cycle has less runway left than the floor; a claim taken late runs deadline-bound and its WIP is preserved for the next run. `CLAIM_REQUIRE_FULL_EXECUTE_BUDGET=1` (env, off by default) additionally refuses claims whose runway cannot fit a full `claude_timeout` execute. |
