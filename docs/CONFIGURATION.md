...
| FLEET health directory           | `fleet_health_dir`                 | _(empty)_  | Directory of the FLEET health tracking checkout (Issue #535). When set, the worker reports health status after work. `setup.sh` / `setup.ps1` ask for it (optional)                                    |
| FLEET health repository          | `fleet_health_repo`                | _(empty)_  | Git URL of the FLEET health repository. Cloned into `fleet_health_dir` when that checkout is missing — by setup, and by the worker on its first run. Never assumed: unset, the worker logs that health tracking is off |
