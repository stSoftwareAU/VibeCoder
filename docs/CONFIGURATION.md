...
| `maintenance_lease_issue` | integer | The issue this repository's maintenance lease marker comments live on (Issue #2450). Normally the anchor issue — titled `Vibe Coder maintenance lease — do not close` — is found or created and pinned automatically; set this to point the fleet at an issue the operator chose instead. Non-positive or non-integer values are ignored. |
