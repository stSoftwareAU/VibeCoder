...
/** Run the installer with the given arguments against a temporary prefix. */
async function runInstaller(args: string[]): Promise<InstallerRun> {
  // The real prefix default (/opt/vibe-tools) is root-owned in the build
  // image; the build itself runs the script as root, but this test does not,
  // so every run gets an isolated, writable prefix instead.
  const prefix = await Deno.makeTempDir({ prefix: "vibe-tools-prefix-" });
  const result = await new Deno.Command("bash", {
    args: [INSTALLER, ...args],
    env: { VIBE_TOOLS_PREFIX: prefix },
    stdout: "piped",
    stderr: "piped",
    stdin: "null",
  }).output();
