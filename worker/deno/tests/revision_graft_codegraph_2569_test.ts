/**
 * Graft, CodeGraph and RTK wiring for the revision phase (Issue #2569).
 *
 * A revision run collects one Graft bundle, prepares CodeGraph and RTK once,
...
import { processIssueRevision } from "../lib/revision_processor.ts";
...
const WORK_DIR = "/tmp/revision-2569-work";

/** Drive one revision round; returns its spawn, posts and collections. */
async function runRevision(
  ...
  const result = await processIssueRevision({
    repo: "org/repo",
    issueNumber: 42,
  ...
  assertEquals(result.ok, true, "the revision round completes");
  return { spawns, posted, collected };
}

Deno.test("revision - wires Graft, CodeGraph and RTK into its spawn and stats comment (Issue #2569)", async () => {
  const { spawns, posted, collected } = await runRevision(
    okGraftOutcome(),
    okCodegraphOutcome(),
    true,
  );
...
Deno.test("revision - all three off leaves the spawn unaccelerated (Issue #2569)", async () => {
  const { spawns, posted } = await runRevision(
    offGraftOutcome(),
    offCodegraphOutcome(),
    false,
  );
