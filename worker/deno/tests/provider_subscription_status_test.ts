...
Deno.test("subscription status preserves multiple windows and resets", () => {
  const status = subscriptionStatusFromQuotaCandidate(
    ...
    1_000,
    { billingMode: "fixed-subscription", confidence: "authoritative" },
  );
...
Deno.test(
  "status cache reuses fresh probe and refreshes stale probe",
  async () => {
    let now = 1_000;
    let calls = 0;
...
Deno.test(
  "status cache converts probe failure to safe unknown without error text",
  async () => {
