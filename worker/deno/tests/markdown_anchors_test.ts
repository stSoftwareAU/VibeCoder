...
Deno.test("anchors - every referencing doc uses the corrected link", async () => {
  for (const ref of REFERENCES) {
...
Deno.test("anchors - the stale pre-fix links are gone", async () => {
  const stale: Array<{ file: string; needle: string }> = [
    {
      file: "docs/SECURITY-SCAN.md",
      needle: "#security-scans-issue-1933-1944)",
    },
    {
      file: "docs/BEST-PRACTICES-SCAN.md",
      needle: "#security-scans-issue-1933-1944)",
    },
    { file: "docs/MODEL-AND-CACHING.md", needle: "(#override-priority)" },
    {
      file: "docs/CONFIGURATION.md",
      needle: "issue-processing.md#issue-selection-priority)",
    },
    { file: "README.md", needle: "USAGE.md#work-prioritisation-order)" },
    { file: "docs/USAGE.md", needle: "(#work-prioritisation-order)" },
    {
      file: "docs/CROSS-REPO-FIX.md",
      needle: "#release-gating-never-auto-release-issue-2944)",
    },
  ];
  for (const { file, needle } of stale) {
    const body = await read(file);
    assert(
      !body.includes(needle),
      `${file} still contains the stale anchor ${needle}`,
    );
  }
});
