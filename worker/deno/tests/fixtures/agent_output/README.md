...
`plugins` / `agents` / `memory_paths` / `messaging_socket_path` /
`mcp_servers` / `capabilities` / `terminal_slash_commands` keys of the `init`
line dropped, and the shell's `Shell cwd was reset …` epilogue removed from
stderr.

## Derived — Claude Code 2.1.261 envelope shapes

No credential was available here, so a *successful* run could not be recorded.
These reuse the recorded envelope shapes above with the message content
changed:
...
| `claude-2.1.261-usage-limit.stderr` | A stderr-only usage-limit refusal carrying the CLI's machine-readable `|<epoch-seconds>` suffix. |
