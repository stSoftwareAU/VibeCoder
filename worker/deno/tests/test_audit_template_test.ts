/**
 * Tests for the test-audit idle-task template (Issue #2251, template #3).
 *
 * Coverage:
 *   - registration: the template is registered at module load and
 *     `getTemplate("test-audit")` / `listTemplates()` find it
 *   - contract flags: cooldownHours === 168, skipMilestone === true,
 *     outputLabel === "test-audit", requiresStructuredOutput === true
 *   - title is the literal "Run a test-audit scan" (used for dispatch)
 *   - buildIssueBody output is recognised by BOTH the title match and
 *     the body fingerprint, and leaves no raw `{{...}}` placeholders
 *   - assembleTestAuditPrompt substitutes placeholders / `(none)` sentinel
 *   - runTask happy path: ensures the `test-audit` label before the scan,
 ...
 *   - claim handler dispatches a "Run a test-audit scan" wrapper to runTask
 *
...
import {
  ...
  renderTestAuditSummary,
  TEST_AUDIT_BODY_FINGERPRINT,
  TEST_AUDIT_ISSUE_TITLE,
  TEST_AUDIT_LABEL,
  testAuditTemplate,
...
const STUB_PROMPT = [
  "# Test-Audit — WHAT-vs-HOW Test Quality Scan (v2)",
  "",
  "Suppressed ids:",
  "{{SUPPRESSED_IDS}}",
  "",
 ...
 * the known-open lookup (`--json number,body,author`). The before/after
 * snapshots return the first/second entry of `snapshots`.
 */
...
Deno.test("test-audit - registered at module load", () => {
  const t = getTemplate("test-audit");
  assert(t !== undefined, "expected test-audit template to be registered");
  assertEquals(t, testAuditTemplate);
  assert(
    listTemplates().some((x) => x.name === "test-audit"),
    "expected listTemplates() to include test-audit",
  );
});

Deno.test("test-audit - contract flags", () => {
  assertEquals(testAuditTemplate.cooldownHours, 168);
  assertEquals(testAuditTemplate.skipMilestone, true);
  assertEquals(testAuditTemplate.outputLabel, TEST_AUDIT_LABEL);
  assertEquals(testAuditTemplate.requiresStructuredOutput, true);
  assertEquals(
    testAuditTemplate.buildIssueTitle("acme/widget"),
    TEST_AUDIT_ISSUE_TITLE,
  );
});

Deno.test("test-audit - buildIssueBody matches title and fingerprint", async () => {
  const t = createTestAuditTemplate({ loadPromptFn: okPrompt });
  ...
  assertEquals(t.buildIssueTitle("acme/widget"), TEST_AUDIT_ISSUE_TITLE);
  ...
  assert(TEST_AUDIT_BODY_FINGERPRINT.test(body));
  ...
  assertStringIncludes(body, "🏷️ Filed by idle-task template: `test-audit`");
...
Deno.test("assembleTestAuditPrompt - empty lists render (none)", () => {
  const out = assembleTestAuditPrompt(STUB_PROMPT, {
    suppressedIds: [],
    knownOpenFindingIds: [],
  });
...
Deno.test("assembleTestAuditPrompt - attribution footer is substituted", () => {
  const footer =
    "🏷️ Filed by idle-task template: `test-audit` · Run id: `vibe-test-id`";
  const out = assembleTestAuditPrompt(STUB_PROMPT, {
    suppressedIds: [],
    knownOpenFindingIds: [],
...
Deno.test("assembleTestAuditPrompt - populated lists are joined", () => {
  const out = assembleTestAuditPrompt(STUB_PROMPT, {
...
Deno.test("assembleTestAuditPrompt - coverage gaps substituted, default (none) (Issue #2916)", () => {
  const promptWithGaps = STUB_PROMPT + "\nGaps:\n{{COVERAGE_GAPS}}";
  // Default: no coverageGaps → (none) sentinel.
  const def = assembleTestAuditPrompt(promptWithGaps, {
    suppressedIds: [],
    knownOpenFindingIds: [],
  });
  assert(!def.includes("{{COVERAGE_GAPS}}"));
  assertStringIncludes(def, "Gaps:\n(none)");
  // Populated: the rendered list is substituted verbatim.
  const populated = assembleTestAuditPrompt(promptWithGaps, {
    suppressedIds: [],
    knownOpenFindingIds: [],
    coverageGaps: "- lib/a.ts:6 — untested",
  });
  assertStringIncludes(populated, "Gaps:\n- lib/a.ts:6 — untested");
});

Deno.test("renderTestAuditSummary - wording", () => {
  assertEquals(renderTestAuditSummary([]), "no findings");
  assertEquals(
    renderTestAuditSummary([12, 3]),
    "Test-audit scan complete. Filed 2 issues: #3, #12",
  );
});
...
Deno.test("runTask - happy path ensures label and diffs snapshot", async () => {
  ...
  assertEquals(
    result.summary,
    "Test-audit scan complete. Filed 2 issues: #7, #9",
  );
  ...
  assert(
    calls.some((c) =>
      c.includes("--label") && c.includes(TEST_AUDIT_LABEL) &&
      c.includes("number")
    ),
  );
});
...
Deno.test("runTask - scan failure surfaces ok:false", async () => {
  ...
  assertStringIncludes(result.summary, "test-audit failed");
...
Deno.test("claim handler - dispatches a test-audit wrapper to runTask", async () => {
  const { gh } = makeGhStub({ snapshots: [[], [11]] });
  const template = createTestAuditTemplate({
    dedupAuthors: DEDUP_AUTHORS,
    ghCommandFn: gh,
    loadPromptFn: okPrompt,
    ensureLabelFn: () => Promise.resolve({ ok: true, value: undefined }),
  ...
  const result = await handleIdleTaskIssue(
    {
      repo: "acme/widget",
      issueNumber: 100,
      issueTitle: TEST_AUDIT_ISSUE_TITLE,
  ...
  assertEquals(
    result.summary,
    "Test-audit scan complete. Filed 1 issues: #11",
  );
});

// ---------------------------------------------------------------------------
// Regression — Issue #2643: a claimed wrapper runs to completion even though
// it is itself an open `idle-task` issue in the target repo.
//
// The retired runTask-level re-check guard (#2441) reused the filer's busy
// set, which counts the `idle-task` label itself. At run time the wrapper
// being executed is always an open, unblocked `idle-task` issue, so the guard
// self-cancelled every run. With the guard removed, the scan must run to
// completion regardless of the wrapper's own presence. The gh stub below
// reports the wrapper as an open `idle-task` issue on the busy-check query
// (`--json number,labels`) — the exact condition that made the old guard
// short-circuit — and the test asserts the scan still ran.
// ---------------------------------------------------------------------------

Deno.test("runTask - runs even though the wrapper is an open idle-task issue (Issue #2643)", async () => {
  let snapshotCount = 0;
  const gh = (args: string[]): Promise<string> => {
    const jsonIdx = args.indexOf("--json");
    const jsonField = jsonIdx >= 0 ? args[jsonIdx + 1] : "";
    if (jsonField === "number") {
      // before: empty, after: one new finding (#42).
      const snap = snapshotCount === 0 ? [] : [42];
      snapshotCount += 1;
      return Promise.resolve(
        JSON.stringify(snap.map((n) => ({ number: n }))),
      );
    }
    if (jsonField === "number,body") return Promise.resolve("[]");
    if (jsonField === "number,labels") {
      // The wrapper itself — an open, unblocked idle-task issue. The old
      // guard treated this as "approved work in flight" and skipped.
      return Promise.resolve(
        JSON.stringify([{ number: 100, labels: [{ name: IDLE_TASK_LABEL }] }]),
      );
    }
    return Promise.resolve("[]");
  };
  ...
  // The scan ran to completion — it was not self-cancelled.
  assert(
    scanRan,
    "expected the scan to run despite the open idle-task wrapper",
  );
  assertEquals(result.ok, true);
  assertEquals(
    result.summary,
    "Test-audit scan complete. Filed 1 issues: #42",
  );
  // The summary must NOT be a skip — the retired guard's wording.
  assert(
