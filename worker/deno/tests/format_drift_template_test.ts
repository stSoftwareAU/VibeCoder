...
Deno.test("assembleFormatDriftPrompt - open issue titles are substituted", () => {
  const out = assembleFormatDriftPrompt(
    "Already open:\n{{OPEN_ISSUE_TITLES}}",
    {
      suppressedIds: [],
      knownOpenFindingIds: [],
...
Deno.test("assembleFormatDriftPrompt - an empty open-issue list renders (none)", () => {
  const out = assembleFormatDriftPrompt(
    "Already open:\n{{OPEN_ISSUE_TITLES}}",
    {
      suppressedIds: [],
