...
Deno.test(
  "validate-scripts.yml still gates Develop, main and milestone/* PRs, through the quality gate (Issue #3333)",
  async () => {
    ...
    // invariant moves with it — the caller carries the branch filter this
    // workflow used to (Issue #3360), and this workflow must be callable.
    const files = await readWorkflowFiles(REPO_ROOT);
    const workflow = files.find((f) => f.path === WORKFLOW_PATH);
    assert(workflow, `expected to read ${WORKFLOW_PATH}`);
    ...
    for (const expected of ["Develop", "main", "milestone/*"]) {
      assertEquals(
        ...
        `quality.yml pull_request.branches must keep \`${expected}\` ` +
          `(Issue #3360); got: ${JSON.stringify(branches)}`,
      );
    }
