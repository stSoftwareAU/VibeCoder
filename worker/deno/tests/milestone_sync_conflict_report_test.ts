 ...
 */
async function setupFixture(collide: boolean): Promise<Fixture> {
  ...
  // The default branch moves on — over the same line when `collide`.
  await gitOk(["checkout", "main"], seed);
  if (collide) {
    await Deno.writeTextFile(`${seed}/scan.ts`, "export const rules = 3;\n");
...
Deno.test(
  "syncMilestoneBranchWithDefault - a conflicting merge reports the files and both sides' commits (Issue #1558)",
  async () => {
    ...
    try {
      ...
      assert(conflict, "a conflicting merge must report its conflict");
      assertEquals(conflict.files, ["scan.ts"]);
      assertEquals(
        conflict.milestoneSha,
        milestoneSha,
        "the milestone side's commit is named",
      );
      assertEquals(
        conflict.defaultSha,
        defaultSha,
        "the default side's commit is named",
      );
    } finally {
      await fx.cleanup();
    }
