...
Deno.test("generateMcpConfig - denies the cloud metadata endpoints to the MCP server process (Issue #1386)", () => {
  const args: string[] =
    JSON.parse(generateMcpConfig({ scriptDir: "/opt/vibe" }))
      .mcpServers.playwright.args;

  // The browser server binds a unix socket whose name carries a fresh guid,
  // and Deno scopes a unix socket by exact path only, so the allow side
  // cannot be scoped without failing every browser_navigate.
  assertEquals(
    args.includes("--allow-net"),
    true,
    "the server must be able to bind its own unix socket",
  );
  assertEquals(
    args.some((a) => a.startsWith("--allow-net=")),
    false,
    "a scoped allow list cannot name the random unix socket path",
  );

  const flag = args.find((a) => a.startsWith("--deny-net="));
  assertEquals(typeof flag, "string", "expected a --deny-net= flag");
  const denied = (flag as string).slice("--deny-net=".length).split(",");
  assertEquals(denied.includes(""), false, "an empty host denies nothing");
  // Deno's deny list beats its allow list, so these stay unreachable from
  // the server process however wide the grant above is.
  for (const host of PLAYWRIGHT_MCP_BLOCKED_HOSTS) {
    assertEquals(denied.includes(host), true, `${host} must stay denied`);
  }
});

Deno.test("deniedNetValue - fails loud on an empty deny list rather than dropping the guard", () => {
  assertThrows(() => deniedNetValue([]), Error, "empty");
});

Deno.test("deniedNetValue - fails loud when a host contains the list separator", () => {
  // A comma would split one entry into fragments that match no host, leaving
  // a list that looks complete and denies nothing.
  assertThrows(
    () => deniedNetValue(["169.254.169.254,evil.example"]),
    Error,
    "comma or whitespace",
  );
  assertThrows(() => deniedNetValue(["evil example"]), Error, "whitespace");
});

Deno.test("generateMcpConfig - propagates an unexpressible denied host instead of emitting a broken flag (Issue #1386)", () => {
  assertThrows(
    () =>
      generateMcpConfig({
        scriptDir: "/opt/vibe",
        deniedNetHosts: ["169.254.169.254", "bad,host"],
      }),
    Error,
    "comma or whitespace",
  );
