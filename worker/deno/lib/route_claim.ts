...
>([
  ...
  "stream_busy",
  // Issue #2336: another host holds this milestone stream's conversation and
  // its head start has not run out. The next scan draws the issue again, and
  // claims it once the grace expires.
  "stream_affinity",
...
function describeRefusal(reason: RouteClaimRefusal): string {
  switch (reason) {
    ...
    case "stream_affinity":
      return "another host holds this milestone stream's conversation";
