 ...
 * `Run a module-doc & README coverage scan` already exists in `repo`.
 * Used to prevent piling new wrappers on top of an un-triaged one. A gh
 * failure is treated as "no open wrapper" so the gate never stalls
 * scanning on a transient hiccup.
 */
async function hasOpenDocCoverageWrapper(
  repo: string,
  ghCommandFn: (args: string[]) => Promise<string>,
): Promise<boolean> {
  let raw: string;
  try {
    raw = await ghCommandFn([
      ...
      "open",
      "--search",
      `"${DOC_COVERAGE_ISSUE_TITLE}" in:title`,
      "--json",
      "number,title",
      "--limit",
      "10",
  ...
  for (const item of parseGhJsonArray(raw, "find doc-coverage wrapper")) {
    if (item === null || typeof item !== "object") continue;
    const title = (item as { title?: unknown }).title;
    if (
      typeof title === "string" &&
      title.trim() === DOC_COVERAGE_ISSUE_TITLE
    ) {
      return true;
    }
  }
...
export function createDocCoverageTemplate(
  ...
  async function shouldFile(
    ...
    if (await hasOpenDocCoverageWrapper(opts.repo, ghCommandFn)) {
      return false;
    }
    return true;
  }
