 ...
 * `Run an orphan-dependency scan` already exists in `repo`. Used to
 ...
 * transient hiccup.
 */
async function hasOpenOrphanDepsWrapper(
  repo: string,
  ghCommandFn: (args: string[]) => Promise<string>,
): Promise<boolean> {
  let raw: string;
  try {
    raw = await ghCommandFn([
      ...
      "open",
      "--search",
      `"${ORPHAN_DEPS_ISSUE_TITLE}" in:title`,
      "--json",
      "number,title",
      "--limit",
      "10",
  ...
  for (const item of parseGhJsonArray(raw, "find orphan-deps wrapper")) {
    if (item === null || typeof item !== "object") continue;
    const title = (item as { title?: unknown }).title;
    if (
      typeof title === "string" &&
      title.trim() === ORPHAN_DEPS_ISSUE_TITLE
    ) {
      return true;
    }
  }
...
export function createOrphanDepsTemplate(
  ...
  async function shouldFile(
    ...
    if (await hasOpenOrphanDepsWrapper(opts.repo, ghCommandFn)) {
      return false;
    }
    return true;
  }
