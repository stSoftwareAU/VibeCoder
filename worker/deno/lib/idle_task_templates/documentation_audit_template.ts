/**
 * Documentation-audit idle-task template (Issue #3319, template #13).
 *
 * Runs a static, evidence-backed audit of the target repository's prose
 * documentation — READMEs, `docs/**`, agent instruction files
 * (`CLAUDE.md`, `AGENTS.md`, …), and the accumulated PR-summary archive —
 * and files one finding issue per surviving concern. Modelled on
 * `test_audit_template.ts`: a single language-agnostic prompt, no bucket,
 * and the same outcome-only Claude contract:
 *
 *   - **Outcome-only Claude contract.** The orchestrating prompt in
 *     `prompts/documentation_audit/` (latest version loaded at runtime)
 *     instructs Claude to file
 *     findings directly via `gh issue create`. `runTask` verifies the
 *     outcome by snapshotting the repo's open `documentation-audit`
 *     issues before and after the scan and diffing them — no JSON
 *     parsing.
 *   - **Label-ensure first.** The `documentation-audit` label is not
 *     seeded anywhere else, so `runTask` ensures it exists before
 *     invoking the scan.
 *   - **Dedup.** The known-open list passed to Claude is built from the
 *     repo's existing open `documentation-audit` issues — Claude is
 ...
 *
 * The scan files **issues only**, never a PR (`skipMilestone: true`).
 * The actual documentation changes — folding PR-summary learnings into
 * the main docs, deleting the now-obsolete summaries, trimming agent
 * files — ride the normal work-on flow on the filed issues.
 *
...
const NAME = "documentation-audit";

const DESCRIPTION =
  "Run the prose-documentation audit against the target repository — " +
  "unify PR-summary learnings into the main docs, prune stale summaries, " +
  "and file each surviving finding as its own issue.";

/** Label every filed documentation-audit finding carries. */
export const DOCUMENTATION_AUDIT_LABEL = "documentation-audit";

/** Static wrapper title — dispatch matches against this string. */
export const DOCUMENTATION_AUDIT_ISSUE_TITLE = "Run a documentation audit";

/** Colour for the `documentation-audit` label (matches the prompt's seed). */
export const DOCUMENTATION_AUDIT_LABEL_COLOUR = "1D76DB";

/** Prompt template directory under `prompts/`. */
const PROMPT_NAME = "documentation_audit";

/** Once-per-week-per-repo cap (enforced by `idle_task_cooldown_gate.ts`). */
const COOLDOWN_HOURS = 168;

/**
 * Body fingerprint that uniquely identifies a documentation-audit
 * wrapper. Anchored to a Markdown heading at start-of-line, matching the
 * `test_audit_template` convention. The prompt's H1 is
 * `# Documentation Audit — Unify Learnings, Prune Stale Docs (vN)` — the
 * fingerprint is version-agnostic (matches any version suffix).
 */
export const DOCUMENTATION_AUDIT_BODY_FINGERPRINT =
  /^#+\s+Documentation Audit\b/m;
...
/**
 * Injectable dependencies for {@link createDocumentationAuditTemplate}.
 *
 ...
 * on Claude.
 */
export interface DocumentationAuditTemplateDeps {
  ...
  /**
   * Documentation-audit scan runner — invokes Claude with the assembled
...
/** Inputs to a documentation-audit Claude run. */
export interface RunScanOptions {
  repo: string;
  workDir: string;
  /** Stable ids already open as `documentation-audit` issues — skip-list. */
...
/**
 * Substitute the placeholders defined by the latest
 * `prompts/documentation_audit/` template.
 *
 ...
 * Pure — no I/O.
 */
export function assembleDocumentationAuditPrompt(
  template: string,
  opts: {
...
/**
 * Return true when an open wrapper titled exactly `Run a documentation
 * audit` already exists in `repo`. Used to prevent piling new wrappers on
 * top of an un-triaged one. A gh failure is treated as "no open wrapper"
 * so the gate never stalls scanning on a transient hiccup.
 */
async function hasOpenDocumentationAuditWrapper(
  repo: string,
  ghCommandFn: (args: string[]) => Promise<string>,
): Promise<boolean> {
  let raw: string;
  try {
    raw = await ghCommandFn([
      ...
      "open",
      "--search",
      `"${DOCUMENTATION_AUDIT_ISSUE_TITLE}" in:title`,
      "--json",
      "number,title",
      "--limit",
      "10",
  ...
  for (
    const item of parseGhJsonArray(raw, "find documentation-audit wrapper")
  ) {
    if (item === null || typeof item !== "object") continue;
    const title = (item as { title?: unknown }).title;
    if (
      typeof title === "string" &&
      title.trim() === DOCUMENTATION_AUDIT_ISSUE_TITLE
    ) {
      return true;
    }
  }
...
export function renderDocumentationAuditSummary(
  ...
  return `Documentation audit complete. Filed ${sorted.length} issues: ${list}`;
}
...
/**
 * Default Claude runner. Loads the latest
 * `prompts/documentation_audit/` template, substitutes placeholders, and invokes Claude with the same write-tool
 * blocklist as `test_audit_template.defaultRunScan` — the scan is
 * read-only and files issues via `gh` only.
 *
...
async function defaultRunScan(
  ...
  const prompt = assembleDocumentationAuditPrompt(promptResult.value, {
  ...
  if (timedOut) {
    return {
      ...
      error: {
        kind: "timeout",
        message: "Claude documentation-audit scan timed out",
      },
    };
  }
  if (exitCode !== 0) {
...
/**
 * Build the documentation-audit template using the supplied deps. Default
 * deps wire production behaviour; tests inject stubs.
 */
export function createDocumentationAuditTemplate(
  ...
  const ensureLabelFn = deps.ensureLabelFn ??
    ((repo) =>
      defaultEnsureLabelExists(
        ...
        DOCUMENTATION_AUDIT_LABEL_COLOUR,
        "Prose-documentation audit finding",
      ));
  ...
  async function buildIssueBody(_opts: IdleTaskBodyOptions): Promise<string> {
    ...
    return assembleDocumentationAuditPrompt(loaded.value, {
      suppressedIds: [],
      knownOpenFindingIds: [],
      attributionFooter,
    });
  ...
  function buildIssueTitle(_repo: string): string {
    return DOCUMENTATION_AUDIT_ISSUE_TITLE;
  }
  ...
  async function shouldFile(
    ...
    if (await hasOpenDocumentationAuditWrapper(opts.repo, ghCommandFn)) {
      return false;
    }
    return true;
  }
  ...
  async function runTask(opts: IdleTaskRunOptions): Promise<IdleTaskRunResult> {
    try {
      // 1. Ensure the `documentation-audit` label exists before any
      ...
      // 2. Snapshot the repo's open documentation-audit issues before any
      ...
      if (!scanResult.ok) {
        return {
          ok: false,
          summary: `documentation-audit failed: ${scanResult.error.kind} — ` +
      ...
      return {
        ok: true,
        summary: renderDocumentationAuditSummary(newlyFiled),
      };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      return {
        ok: false,
        summary: `documentation-audit threw: ${message}`,
      };
    }
  }
  ...
  return {
    ...
    skipMilestone: true,
    outputLabel: DOCUMENTATION_AUDIT_LABEL,
...
export const documentationAuditTemplate: IdleTaskTemplate =
  createDocumentationAuditTemplate();

registerTemplate(documentationAuditTemplate);
