...
async function findOpenEscalationIssue(
  ...
  const verified = await selectFleetAuthoredMatches(
    rows.filter((row) => isPrBranchFailureIssue(row.body ?? "", repo, branch)),
    `pr-branch-update ${prBranchFailureKey(repo, branch)}`,
    opts,
    log,
  );
...
async function decide(
  ...
  try {
    existing = await findOpenEscalationIssue(repo, branch, opts, log);
  } catch (err) {
    const reason = err instanceof Error ? err.message : String(err);
    log(
