...
4. **Retry on the next cycle** within a bounded attempt budget — **two** judged
   ...
   that judged the conflict is charged.
...
- **Grill-me extension.** The `grill_me` phase routes to the same Fable 5 top
  tier as planning, so the stats/verdict helpers are phase-parametric and
  grill-me reuses them — but applies `degraded-model` **on a degraded round**
...
`{ claude: "2.1.260" }` — the oldest release that resolves the `fable` alias to
Fable 5.1 _and_ carries its prompt-cache fixes, Issue #1362). The map is generic
per tool so `gh`/`deno` floors can be added later.
...
[`chain_root_comment.ts`](worker/deno/lib/chain_root_comment.ts) (the comment
and its 24-hour dedup). Operator view:
