...
| — [Merge-conflict Resolution](docs/workflows/merge-conflicts.md)              | Real merge of the base into a conflicting PR — both sides survive unless an originating issue explicitly supersedes the other — deterministic dependency rules before the agent, plus attempt bounds and escalation |
...
| **[Extending the Worker](docs/EXTENDING.md)**                                  | Adding Deno/TypeScript commands, editing prompt templates, shell integration                                                                                  |
| **[Private Extensions](docs/PRIVATE-EXTENSIONS.md)**                           | Extending a deployment with software this repository does not ship — installing arbitrary toolchains, configuring them, and directing an agent across them, all from gitignored configuration rather than a fork |
| **[Container Extension](docs/CONTAINER-EXTENSION.md)**                         | The image-side extension point: a private layer built on the standard image for services and toolchains of no use to the general public, with a worked example, the layering contract, the rebuild rule, service supervision, and every failure mode's exact symptom |
| **[Custom Label Prompts](docs/CUSTOM-PROMPTS.md)**                             | The operator-side extension point: mapping a GitHub label to a prompt template that lives outside this repository, with a worked example, the placeholder contract, the trust gate, the read-only container mount, and every failure mode's exact symptom |
...
| **[Prompt house vocabulary](docs/PROMPT-HOUSE-VOCABULARY.md)**                 | The house form of every term and shared section heading across the prompt set: banned variants, why each form won, and the deliberate exceptions |
...
| **[Repo-context Trial](docs/REPO-CONTEXT-TRIAL.md)**                           | Protocol for the Graft and CodeGraph repo-context trial: the candidates, the shared bar, the sequential windows on one host, the exclusion rules, where every figure is read from, why Graphify was dropped, and the per-candidate result tables |
| **[RTK output trial](docs/RTK-OUTPUT-TRIAL.md)**                               | Protocol for the RTK Bash-output trial behind `rtk_output.enabled`: the candidate and how it is wired, the bar, the human-opened window, the comparison rule, where every figure is read from, the security posture, and the comparable tools recorded but not trialled |
| **[brief trial](docs/BRIEF-TRIAL.md)**                                         | Protocol for the brief trial behind `brief_toolchain.enabled`: the candidate and how it is wired, the bar, the human-opened window, the comparison rule, where every figure is read from, the security posture, and the verdict template |
| **[Codex Budget Sources](docs/CODEX-BUDGET-SOURCES.md)**                       | What the pinned Codex CLI actually exposes about remaining quota, verified against its source: the rollout `token_count` snapshot, explicit exhaustion evidence, subscription vs API-key billing, and what was rejected and why |
| **[Codex subscription auth](docs/CODEX-SUBSCRIPTION-AUTH.md)**                  | Persistent ChatGPT login for unattended Codex runs: where the refreshed auth state survives a container replacement, and the billing guard that withholds `OPENAI_API_KEY` / `CODEX_API_KEY` from a subscription run |
| **[Provider parity](docs/PROVIDER-PARITY.md)**                                 | The subscription-only billing policy, shared quota scheduler, scoped usage pauses, multiple Codex credentials, opt-in fallback, and the staged Codex rollout / rollback runbook |
| **[Subscription soak](docs/SUBSCRIPTION-SOAK.md)**                             | Restart/soak qualification for subscription providers: automated restart-cycle coverage, the operator-facing observability surface, and the manual soak procedure a release engineer runs before `auto` routing |
| **[GitHub API Optimisation](docs/GH-API-OPTIMISATION.md)**                     | `gh` cache layers, list-then-filter pattern, cross-repo PR prefetch, GraphQL batch path, invalidation rules, per-iteration call telemetry              |
...
| **[Spec-Kit Comparison](docs/SPEC-KIT-COMPARISON.md)**                         | Point-in-time assessment of GitHub spec-kit against this workflow: the phase-by-phase mapping, five ideas adopted natively (one since removed by design), five deliberately rejected |
...
| **[Documentation-Audit Scans](docs/DOCUMENTATION-AUDIT-SCAN.md)**              | Operator manual for the prose-documentation audit: unify PR-summary learnings into the README, prune stale docs, remove comments the code contradicts, the fourteen checks, labels, suppression |
...
| **[Gate-Skip Drift Scans](docs/GATE-SKIP-DRIFT-SCAN.md)**                      | Operator manual for the weekly gate-skip drift audit: a repo's `quality.sh` skipping a tool its own CI installs and runs, the `container/tools.json` suppression, and the two fixes |
...
| **Merge conflict** | `merge-conflict` | — (hardwired) | Applied by the worker to an open PR whose branch conflicts with its base, so the stuck queue is visible at a glance instead of buried in per-pass logs. The Priority 1.61 conflict pass then merges the base in for real — both sides' changes survive, never a side-pick, and the two bounded carve-outs (dependency versions, and an issue that explicitly supersedes the other side's) are evidenced and named on the PR — runs the repo's quality gate, and pushes. The label is removed once the PR merges cleanly again; after two failed attempts the worker **abandons and restarts** — it closes the PR with an explanatory comment and re-queues its originating issue so a fresh PR is raised off the current base — twice per issue, and it escalates with `needs-human` only when that restart is declined for another reason or fails part-way. Once both restarts are spent the PR is **parked** instead: it stays open carrying `merge-conflict` with a marker naming the base tip it is waiting on, and is attempted again — with a fresh two-attempt budget — only when that tip moves. Every attempt ends visibly — merged, failed, or escalated — and an attempt disrupted before it concluded is re-attempted rather than counted, with its own 3-disruption bound. |
| **Merge fallback** | `merge-fallback` | — (hardwired) | Applied by the worker to the issue it files when a merge conflict **falls back** to the last known good state — the PR path closes the PR, the milestone path reverts its merged children. The flag names the target, the conflicted files, both agent runs' analyses, each run's per-stage timings and host, how far behind the base the branch was and since when, and what was closed or reverted; a field the fallback could not record says `not recorded` rather than disappearing. One flag per PR or milestone branch — a repeat fallback is appended to the open flag as a comment — and it gains `idle-task` only when the flag is itself the re-do work item (a conflicting PR whose originating issue cannot be found). See [Merge-conflict resolution](docs/workflows/merge-conflicts.md#-every-fallback-is-flagged). |
...
| Label                   | Default               | Config Key                  | Description                                                                                                                                 |
| ----------------------- | --------------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Failed once**         | `failed-once`         | `failed_once_label`         | Applied after the first non-transient processing failure — coding, planning and question runs alike (Issue #1949). The issue is unassigned and automatically retried on the next scan. Transient infrastructure consumes no attempt — account state (rate/usage limit, out of credit, interruption, scheduled release, deadline-bound timeout) and host state (OOM, full disk, worker crash, missing tools, external kill). |
